...
exports.MoodSelectionModel = void 0;
const database_1 = require("../config/database");
class MoodSelectionModel {
    // Get all mood selections for a user
    static async getUserMoodSelections(userId, limit) {
        try {
    ...
    static async getLatestMoodSelection(userId, page = 'main') {
        try {
            const client = await database_1.pool.connect();
    ...
    static async createOrUpdateMoodSelection(userId, moodData) {
        try {
            const client = await database_1.pool.connect();
    ...
    static async getMoodSelectionsByPage(page, limit) {
        try {
            const client = await database_1.pool.connect();
            let query = `
    ...
    static async getUserMoodStats(userId, days) {
        try {
            const client = await database_1.pool.connect();
            let dateFilter = '';
            const params = [userId];
    ...
    static async getAllMoodSelectionsWithUsers(limit) {
        try {
            const client = await database_1.pool.connect();
            let query = `
        SELECT 
    ...
    static async deleteMoodSelection(id) {
        try {
            const client = await database_1.pool.connect();
    ...
    static async deleteUserMoodSelections(userId) {
        try {
            const client = await database_1.pool.connect();
    ...
    static async shouldShowMoodSelector(userId, page = 'main') {
        try {
            const client = await database_1.pool.connect();
            const result = await client.query(`
        SELECT selected_at 
...
exports.MoodSelectionModel = MoodSelectionModel;
//# sourceMappingURL=MoodSelection.js.map